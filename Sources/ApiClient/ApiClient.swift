...
public class ApiClient: NetworkClient {
    ...
    public func request<T>(_ request: CoreModule.Request, queue: DispatchQueue = .main, completion: @escaping (Result<T, NetworkError>) -> Void) where T: Decodable {
        guard let request = try? buildRequest(from: request) else {
            return completion(.failure(.invalidRequest))
        }
        AF.request(request).validate().responseDecodable(of: T.self, queue: queue) { response in
            switch response.result {
            case let .success(decodableResult):
